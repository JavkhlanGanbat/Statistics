...
class LogisticRegressionGD:
    def __init__(self, lr=0.1, lr_decay=1e-4, n_iter=800, l2=1e-4, random_state=42):
        self.lr = lr
        self.lr_decay = lr_decay
        self.n_iter = n_iter
        self.l2 = l2
    ...
    def _loss(self, y, p):
        ...
        reg = (self.l2 / (2 * m)) * np.sum(self.weights * self.weights)
        return ce + reg
    ...
    def fit(self, X, y):
        ...
        self.weights = rng.normal(scale=0.01, size=(n, 1))
        self.bias = 0.0
        for i in range(self.n_iter):
            lr_t = self.lr / (1 + self.lr_decay * i)
            z = X @ self.weights + self.bias
            p = self.sigmoid(z).ravel()
            err = (p - y).reshape(-1, 1)
            dw = (X.T @ err) / m + (self.l2 / m) * self.weights
            db = float(err.sum() / m)
            self.weights -= lr_t * dw
            self.bias -= lr_t * db
            self.losses.append(self._loss(y, p))
        return self

    def predict_proba(self, X):
        z = X @ self.weights + self.bias
        p1 = self.sigmoid(z).ravel()
